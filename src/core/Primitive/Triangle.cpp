...
const pointf2 Triangle::LerpUV(float w, float u, float v) const {
  ...
  return pointf2::combine(array{texcoords[indices[0]], texcoords[indices[1]],
                                texcoords[indices[2]]},
                          array{w, u, v});
}
...
const pointf3 Triangle::LerpPosition(float w, float u, float v) const {
  ...
  return pointf3::combine(array{positions[indices[0]], positions[indices[1]],
                                positions[indices[2]]},
                          array{w, u, v});
}
...
const normalf Triangle::LerpNormal(float w, float u, float v) const {
  ...
  normalf n = w * normals[indices[0]] + u * normals[indices[1]] +
              v * normals[indices[2]];
  return n.normalize();
}
...
const bboxf3 Triangle::Box() const {
  assert(mesh != nullptr);
  const auto& positions = mesh->positions.get();
  bboxf3 b = bboxf3::minmax(positions[indices[0]], positions[indices[1]])
                 .combine(positions[indices[2]]);
